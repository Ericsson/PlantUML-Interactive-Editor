...
- **POST /editParticipantName** — Input: `plantuml`, `svg`, `name`, `svgelement`. Returns: modified puml. `name` is the new *displayed* name; its whitespace is collapsed to single spaces (a declaration is one line, so an embedded newline would split it) and a blank name returns the puml unchanged. What changes depends on the declaration: an aliased participant (`participant "Old" as ON`) has only its displayed name replaced, since the body refers to the alias; an unaliased one has its declaration and every reference updated; and if the new name cannot be a bare reference token (it contains whitespace, `#`, `,` or `:`) the participant gains a generated PascalCase alias (`participant "Space Room" as SpaceRoom`) that the references switch to. Only real reference positions are rewritten — message endpoints, `activate`/`deactivate`/`destroy`/`create`, note placement, `ref over` — so message text and note bodies that mention the old name are left alone.
