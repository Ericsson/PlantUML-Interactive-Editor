...
- **getSequencePositions:** `{plantuml, svg}`; returns `{"participants": [{name, cx, yTop, yBottom, index}, ...], "messages": [{cy, index, text}, ...], "notes": [{cy, index}, ...], "groups": [{headerIndex, endIndex}, ...]}` — called once per render; bundles every sequence element type's position table into one response so a render costs a single round-trip. Fetched into `participantLifelines`, `messagePositions`, `notePositions`, `groupPositions`. Powers hover detection/highlighting, the ghost arrow, and the activation/group snapping gestures
...
- **getSeqNoteText:** `{plantuml, svg, svgelement}`; returns `{"text": note_text, "noteType": "note"|"hnote"|"rnote"}` — fetches current note text and detected type for the edit modal
- **editSeqNote:** `{plantuml, svg, svgelement, text, noteType?}`; returns `{"plantuml": updated_puml}` — replaces the note text and, if `noteType` differs from the note's current type, rewrites its keyword
