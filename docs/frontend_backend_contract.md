...
- **editParticipantName:** `{plantuml, svg, name, svgelement}`; returns `{"plantuml": updated_puml}` — `name` is the new *displayed* name. An aliased declaration (`participant "Old" as ON`) has only its displayed name replaced; an unaliased one has its declaration and every reference updated; and a name that cannot be a bare reference token (whitespace, `#`, `,` or `:`) makes the participant gain a generated PascalCase alias (`participant "Space Room" as SpaceRoom`) that the references switch to. A participant introduced implicitly by a message gets a declaration line inserted at its first use. Only reference positions are rewritten, so message text and note bodies mentioning the old name are preserved
...
- **addNote:** `{plantuml, svg, participant, placement, text, yPosition, secondParticipant?, noteType?}`; returns `{"plantuml": updated_puml}` — inserts a note at the Y-position; placement is 'over', 'left', 'right', or 'spanning'; noteType is 'note', 'hnote', or 'rnote' (defaults to 'note' if missing/unrecognized); all three types support the same placement grammar. `participant`/`secondParticipant` are *displayed* names (what the SVG renders and `participantLifelines` carries); the backend resolves each to the alias the diagram body uses
- **getSeqNoteText:** `{plantuml, svg, svgelement}`; returns `{"text": note_text, "noteType": "note"|"hnote"|"rnote", "color": note_color}` — fetches current note text, detected type, and background color (leading `#` stripped, `""` when uncolored) for the edit modal; handles single-line (`note over A : text`) and block (`note over A` ... `end note`) notes, returning a block note's body joined by real newlines
- **editSeqNote:** `{plantuml, svg, svgelement, text, noteType?, color?}`; returns `{"plantuml": updated_puml}` — replaces the note text and, if `noteType` differs from the note's current type, rewrites its keyword; `color` sets the note background as a `#color` token (e.g. `note over A #LightBlue : text`), omitted/`null` leaves any existing color unchanged, `""`/`"none"` removes it; for a block note the text replaces the body and the block form (including `end note`) is preserved
- **deleteSeqNote:** `{plantuml, svg, svgelement}`; returns `{"plantuml": updated_puml}` — removes the note from puml (the whole `note` ... `end note` block for block-form notes)
- **addActivation:** `{plantuml, participant, startMessageIndex, endMessageIndex, endType}`; returns `{"plantuml": updated_puml}` — inserts a matched `activate` line after the message at `startMessageIndex` and a closing `deactivate`/`destroy` line after the message at `endMessageIndex`; `endType` is 'deactivate' or 'destroy' (defaults to 'deactivate'). `participant` is the *displayed* name; the backend resolves it to the alias the diagram body uses
