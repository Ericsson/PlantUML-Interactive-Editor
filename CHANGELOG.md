...
- Participant boxes for sequence diagrams: right-click a participant → Box, then hover another participant to grow a live ghost box across the range and click to create it. Boxes wrap the selected contiguous participants in a `box ... end box` block (currently created untitled and uncolored). Nested boxes are supported — when the range nests inside or contains an existing box, `!pragma teoz true` is added automatically so PlantUML renders the nesting; a range that crosses an existing box is rejected with an error. Delete a box by right-clicking it → Delete Box, which unwraps it (removing only the `box`/`end box` lines) and keeps the participants. Boxes also participate in bidirectional hover highlighting.
