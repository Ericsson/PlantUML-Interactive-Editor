...
- Participant boxes for sequence diagrams: right-click a participant → Box, then hover another participant to grow a live ghost box across the range and click to create it. Boxes wrap the selected contiguous participants in a `box ... end box` block. Nested boxes are supported — when the range nests inside or contains an existing box, `!pragma teoz true` is added automatically so PlantUML renders the nesting; a range that crosses an existing box is rejected with an error. Right-click a box → Edit Box to set a title and pick a background color (preset palette or None), or → Delete Box to unwrap it (removing only the `box`/`end box` lines) and keep the participants. Boxes also participate in bidirectional hover highlighting.
