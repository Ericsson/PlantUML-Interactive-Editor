...
- Renaming a sequence-diagram participant to a name containing spaces now works. The participant is given a short alias automatically (`participant "Space Room" as SpaceRoom`) and the messages, notes and activations that refer to it are switched to that alias, so the diagram still renders. Renaming a participant that already has an alias changes only its label, leaving the body alone. A participant that was never declared on its own line (introduced only by a message) gets a declaration inserted at its first use.
- Renaming a participant no longer rewrites text that merely mentions the old name: message text, note bodies, group labels and comments are left untouched, and only real references (message endpoints, `activate`/`deactivate`/`destroy`/`create`, note placement, `ref over`) are updated. Previously the rename was a blanket search-and-replace over the whole diagram, so `Alice -> Bob: ask Alice first` had its text rewritten too.
- Pressing Enter in the participant rename field now submits instead of inserting a newline. A participant declaration is a single puml line, so the newline broke the diagram. Whitespace in a submitted name is collapsed (so a pasted multi-line name is handled too), and a blank name leaves the diagram unchanged.
...
- Added `sequence/rename.py`, which owns participant renaming as pure puml surgery (no SVG): the three declaration cases, PascalCase alias generation with sanitizing and collision suffixes, and token-aware reference rewriting. `participant.edit_participant_name` is now just the click-resolution adapter that delegates to it. Covered by `tests/sequence/test_participant_rename_alias.py`, `tests/sequence/test_participant_rename_references.py` and a `TestRenameParticipant` class in `tests/sequence/test_participant.py`.
- Modelled participant declarations in `sequence/classes.py`: `ParticipantDeclaration` plus `parse_participant_declaration`/`participant_declarations` split a declaration into keyword, displayed name, alias and trailing modifiers, and `PARTICIPANT_DECLARATION_RE` now covers every lifeline keyword (`participant`, `actor`, `boundary`, `control`, `entity`, `database`, `collections`, `queue`) rather than `participant` alone. `Participant` gained `alias` and a `reference_name` property. Covered by `tests/sequence/test_participant_declarations.py`.
- Fixed the displayed-name/alias mismatch in every writer that names a participant: `message.add_message`, `activation.add_activation`/`delete_activation`, `note.add_note` and `participant.delete_participant` now use the reference name (the alias when there is one) instead of the displayed name, via `Participant.reference_name` or the new `classes.reference_name_for`. Adding a message, activation bar or note to `participant "Space Room" as SpaceRoom` previously emitted `Space Room -> Bob`, which PlantUML rejects, and deleting such a participant left its notes behind. No frontend change: the routes still receive displayed names and resolve them server-side. Covered by `tests/sequence/test_aliased_participant_operations.py`.
- Fixed `extract_participant_rect` in `tests/sequence/test_participant.py`, which picked the first `<rect>` regardless of type: in a diagram with an activation bar it returned the bar and the "clicked participant" resolved to whichever lifeline the bar sat on. It now filters with `is_participant_rect`, the same way the backend counts clicked participants.
...
- Rewrote `Diagram._assign_participant_indexes` (`sequence/classes.py`) to map each participant to its declaration line by displayed name (quoted or bare, via the new `PARTICIPANT_DECLARATION_RE`/`parse_participant_declaration`) rather than by position. Implicitly-introduced participants correctly keep index `-1` ("no declaration line") instead of shifting every later participant onto the wrong line, and `add_box` now rejects a negative index rather than letting Python's negative indexing invert the box. Added `tests/sequence/test_participant_indexes.py` and implicit-participant cases in `tests/sequence/test_box.py`.
