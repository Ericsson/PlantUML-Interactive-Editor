...
- Increased the retry budget of `_right_click_until_menu` in `tests/e2e/test_sequence_note_menu.py` (5 → 8 attempts, 3s → 5s per-attempt timeout) to reduce flaky CI failures from the sequence demo's post-load re-render race.
