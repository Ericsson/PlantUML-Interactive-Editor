...
- Renaming a sequence-diagram participant to a name containing spaces now works: the participant is given a short alias automatically (`participant "Space Room" as SpaceRoom`) and the messages, notes and activations referring to it follow. Renaming one that already has an alias changes only its label.
- Renaming a participant no longer rewrites text that merely mentions the old name, such as message text and note bodies; only real references are updated.
- Fixed interactive editing of a sequence-diagram participant whose label is broken over several lines (`participant "a\nb" as ab`): renaming and deleting it did nothing, "add participant left/right" inserted at the end of the file, and hovering it highlighted no editor line. Renaming another participant *to* a multi-line name now works too.
- The participant rename field now accepts multi-line names: Enter inserts a line break, saved as the `\n` escape PlantUML draws as a broken label (`participant "New\nName" as NewName`). Ctrl+Enter submits.
...
- `Participant.from_svg` now reads a header's label via the new `classes.participant_label`, which walks every `<text>` sibling of the rect rather than only the first, so a broken label no longer comes back as just its first line and matches its declaration again. Comparison goes through `display_names_match`, since the SVG does not record which break escape produced a break. Covered by `tests/sequence/test_participant_multiline_name.py`.
- Added `sequence/rename.py`, which owns participant renaming as pure puml surgery: the three declaration cases, PascalCase alias generation with sanitizing and collision suffixes, and token-aware reference rewriting. A participant with no declaration to rewrite gets one inserted at its first use, and `_normalize_incoming_name` folds a real newline into the literal `\n` escape (each line trimmed, its whitespace collapsed) rather than splitting the declaration. `participant.edit_participant_name` is now just the click-resolution adapter. Covered by `tests/sequence/test_participant_rename_alias.py`, `test_participant_rename_references.py` and `TestRenameParticipant`.
- Modelled participant declarations in `sequence/classes.py`: `ParticipantDeclaration` and `parse_participant_declaration` split a declaration into displayed name, alias and trailing modifiers, which `PARTICIPANT_DECLARATION_RE` previously discarded. Only the `participant` keyword is parsed; the others are not clickable, and parsing them let an `actor "Alice" as A` claim the line of a separate `participant Alice`. `Participant` gained `alias` and `reference_name`.
- Fixed the displayed-name/alias mismatch in `message.add_message`, `activation.add_activation`/`delete_activation`, `note.add_note` and `participant.delete_participant`: naming a participant by its displayed name emitted the invalid `Space Room -> Bob` for `participant "Space Room" as SpaceRoom`, and left its notes behind on delete. They now use the reference name; the routes still receive displayed names and resolve them server-side, so no frontend change. Covered by `tests/sequence/test_aliased_participant_operations.py`.
- Fixed `extract_participant_rect` in the participant tests, which picked the first `<rect>` regardless of type and so returned an activation bar instead of a participant header.
