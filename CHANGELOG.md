...
- Added a collapse/expand toggle for the code editor pane: click the chevron button on the pane divider, or drag the divider past the minimum width, to collapse the editor to a thin strip (giving the diagram more width). Expand it back by clicking the chevron again or dragging the divider back out. The Ace editor instance stays mounted while collapsed, so its content and state are preserved. The state is not persisted across page reloads.
