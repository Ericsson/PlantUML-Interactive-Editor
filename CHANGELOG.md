...
- Added a `replaced` field to the webview's `documentChanged` message (`static/vscode/webviewInit.js`). It marks a source the host is handing over because the panel was pointed at a different diagram or file, as opposed to an edit of the one on screen, and the page then seeds it with `primeDocumentText` and calls `renderPlantUml()` directly — the path the first document already took. That keeps `debouncedRenderPlantUml`'s closing `setEditorMarkers(findChangedLines())` on the lines the user edited, where arriving at an unrelated diagram had it mark almost every line of the new one. Older pages ignore the field and behave as before; an older host sends none, which reads as false. The extension side is in [plantuml-extension/CHANGELOG.md](plantuml-extension/CHANGELOG.md).
