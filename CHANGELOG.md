...
- Increased the retry budget of `_right_click_until_menu` in `tests/e2e/test_sequence_note_menu.py` (5 → 8 attempts, 3s → 5s per-attempt timeout) to reduce flaky CI failures from the sequence demo's post-load re-render race. `_open_sequence_demo` now also waits for `networkidle` (and re-confirms the lifelines) after switching diagrams, so the burst of background render/positions requests — including the activity diagram being replaced still finishing its calls — drains before the first real right-click; previously the click could land mid-burst on an element whose context-menu handler was not yet (re)attached, so `#sequence-menu` never opened.
